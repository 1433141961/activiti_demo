...
<div class="sbox">
    ...
    <div class="cont">
		<h2 title="activiti请假系统">activiti请假系统</h2>
        <form method="post" id="loginForm" action="<%=request.getContextPath()%>/user/selectUserByUserNameAndPassword.do">
            <ul class="uls set">
            <c:if test="${message != null}">
                <li class="errorTip"><c:out value="${message}"/></li>
            </c:if>
            <li>
                <label class="required desc">用户名：</label>
                <input type="text" class="text medium" name="userName" tabindex="1" />
            </li>
            <li>
                <label class="required desc">密码：</label>
            ...
            <li><label></label>
                <input type="submit" id="go" class="hand btn102x26" name="login" value="登录" tabindex="3" />
            </li>
